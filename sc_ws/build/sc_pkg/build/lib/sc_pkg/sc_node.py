...
from sc_msg.msg import Commands as SCcommands
from sc_msg.msg import Status as SCstatus
...
class IKADriverROS(Node):
    ...
    def setRPM(self, rpm, duration):
        ...
        current_time = self.get_clock().now().strftime("%H:%M:%S")
        self.get_logger().info(f"Setting RPM to: {rpm}")
        self.get_logger().info(
            f"Spin coating for {duration} minutes started at {current_time}"
        )
        rclpy.spin_once(self, timeout_seconds=duration * 60)  # Blocking for duration
    ...
    def callback_commands(self, msg):
        if msg.apex_SpinCoater_Commands == msg.VACUUMON:
            self.turnVacuumON()
        elif msg.apex_SpinCoater_Commands == msg.VACUUMOFF:
            self.turnVacuumOFF()
        elif msg.apex_SpinCoater_Commands == msg.SPINCOAT:
            self.setRPM(msg.apexSC_rpm, msg.apexSC_coatingDuration)
            self.stopCoating()
        elif msg.apex_SpinCoater_Commands == msg.STOP:
            self.stopCoating()
        else:
